...
function categoryNames() { return [...new Set(menus.map(item => item.category || "อื่น ๆ"))]; }
function orderedCategories() { const names = categoryNames(); const known = categoryOrder.filter(name => names.includes(name)); const unknown = names.filter(name => !known.includes(name)).sort((a, b) => a.localeCompare(b, "th")); return [...known, ...unknown]; }
...
async function persistCategoryOrder({ silent = false } = {}) {
  ...
  try { await dataService.saveStoreSettings({ categoryOrder: nextOrder }); if (!silent) toast("บันทึกลำดับหมวดหมู่แล้ว"); }
  catch (error) { console.error("SAVE_CATEGORY_ORDER_FAILED", error); toast("บันทึกลำดับหมวดหมู่ไม่สำเร็จ", "error"); }
  finally { savingCategoryOrder = false; setButtonBusy(saveCategoryOrderButton, false, "กำลังบันทึก...", "บันทึกลำดับหมวด"); }
}
...
async function persistItemOrder({ silent = false } = {}) {
  ...
  try { await Promise.all(ids.map((id, index) => dataService.saveMenu({ id, sortOrder: index + 1 }))); if (!silent) toast(`บันทึกลำดับเมนูในหมวด ${selectedCategory} แล้ว`); }
  catch (error) { console.error("SAVE_ITEM_ORDER_FAILED", error); toast("บันทึกลำดับเมนูไม่สำเร็จ", "error"); }
  finally { savingItemOrder = false; setButtonBusy(saveItemOrderButton, false, "กำลังบันทึก...", "บันทึกลำดับเมนู"); }
}
...
function renderItems() {
  ...
  const items = menus.filter(item => (item.category || "อื่น ๆ") === selectedCategory).sort((a, b) => Number(a.sortOrder || 9999) - Number(b.sortOrder || 9999) || String(a.name || "").localeCompare(String(b.name || ""), "th"));
  itemList.innerHTML = items.length ? items.map((item, index) => `<div class="sort-item" data-menu-id="${escapeHtml(item.id)}"><span class="sort-handle" aria-hidden="true">⋮⋮</span><div><strong>${escapeHtml(item.name)}</strong><div class="menu-category">${item.active !== false ? "เปิดขาย" : "ปิดขาย"} • ${Number(item.price || 0).toLocaleString("th-TH")} บาท</div></div><span class="sort-order-badge">${index + 1}</span></div>`).join("") : '<div class="sort-empty">ยังไม่มีเมนูในหมวดนี้</div>';
  itemSortable?.destroy();
  if (window.Sortable && items.length) itemSortable = new window.Sortable(itemList, { animation: 180, handle: ".sort-handle", ghostClass: "sort-ghost", chosenClass: "sort-chosen", dragClass: "sort-drag", delay: 120, delayOnTouchOnly: true, touchStartThreshold: 4, onEnd: async () => { refreshOrderBadges(itemList); await persistItemOrder({ silent: true }); } });
}
...
function scheduleSortRefresh() { if (!sortManagerReady) return; clearTimeout(refreshTimer); refreshTimer = setTimeout(() => loadSortManager().catch(error => { console.error(error); toast("อัปเดตรายการจัดลำดับไม่สำเร็จ", "error"); }), 120); }
