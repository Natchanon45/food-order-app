...
els.items.addEventListener("focusin",event=>{event.target.closest(".return-qty")?.select()});
els.items.addEventListener("mouseup",event=>{const input=event.target.closest(".return-qty");if(!input)return;event.preventDefault();input.select()});
els.items.addEventListener("keydown",event=>{const input=event.target.closest(".return-qty");if(!input||event.key!=="Enter")return;event.preventDefault();updateTotal();const inputs=[...els.items.querySelectorAll(".return-qty:not(:disabled)")],next=inputs[inputs.indexOf(input)+1];if(next){next.focus();next.select();next.scrollIntoView({block:"nearest",behavior:"smooth"})}});
