...
function renderPaymentHistory(items=[]) { state.payments=items||[]; if(!els.paymentHistory)return; if(!state.payments.length){els.paymentHistory.innerHTML=`<div class="report-history-empty">${t("payment.empty")}</div>`;return;} els.paymentHistory.innerHTML=state.payments.map(item=>{ const date=item.submittedAt?new Date(item.submittedAt).toLocaleString(locale==="th"?"th-TH":"en-US",{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}):"-"; const status=["pending","approved","rejected"].includes(item.status)?item.status:"pending"; return `<article class="report-history-item"><div class="report-history-main"><strong>${escapeHtml(item.period?.label||"-")}</strong><small>${escapeHtml(t("payment.submittedAt",{date}))}</small></div><div class="report-history-value"><span>${t("payment.amountDue")}</span><strong>${money(item.revenueShareAmount)} ${t("common.baht")}</strong></div><div class="report-history-value"><span>${escapeHtml(t("payment.rate",{rate:money(item.revenueShareRate)}))}</span><span class="report-status ${status}">${escapeHtml(statusLabel(status))}</span></div>${item.reviewNote?`<div class="report-history-note"><strong>${escapeHtml(t("payment.reviewNote"))}</strong><span>${escapeHtml(item.reviewNote)}</span></div>`:""}<div class="report-history-actions"><button class="btn btn-sm" type="button" data-report-view-slip="${escapeHtml(item.id)}"><i class="bi bi-receipt"></i><span>${t("actions.viewSlip")}</span></button></div></article>`; }).join(""); }
...
async function submitPayment(event) {
  ...
  try {
    ...
    await callSubmit({ ...periodPayload(), paymentId, slipPath:path, slipName:file.name });
    els.paymentForm.reset(); clearSelectedSlip(); setPaymentMessage(t("states.uploadSuccess")); await loadPaymentHistory();
  } catch(error) {
