...
const callDeletePayment = httpsCallable(functions, "deleteTenantRevenueSharePayment");
...
const TEXT = {
  th: {
    ...
    "actions.refresh":"รีเฟรช","actions.submitSlip":"ส่งสลิปเพื่อตรวจสอบ","actions.viewSlip":"ดูสลิป","actions.deleteSlip":"ลบสลิป","actions.previewSelected":"ดูไฟล์ที่เลือก","actions.removeSelected":"ลบไฟล์","actions.close":"ปิด",
  ...
  en: {
    ...
    "actions.refresh":"Refresh","actions.submitSlip":"Submit slip for review","actions.viewSlip":"View slip","actions.deleteSlip":"Delete slip","actions.previewSelected":"Preview selected file","actions.removeSelected":"Remove file","actions.close":"Close",
...
function ocrMessage(ocr={}) { const missing=ocr?.detectedAmount===null||ocr?.detectedAmount===undefined; const amount=missing?"-":money(ocr.detectedAmount); if(ocr?.status==="manual_review"&&missing)return t("payment.ocrManual"); if(missing)return t("payment.ocrUnreadable"); if(ocr?.amountMatched===true)return t("payment.ocrMatched",{amount}); return t("payment.ocrMismatch",{amount}); }
function recipientMessage(ocr={}) { const recipient=String(ocr?.expectedRecipientName||"").trim(); if(!recipient)return t("payment.recipientNotConfigured"); if(ocr?.recipientMatched===true)return t("payment.recipientMatched",{recipient}); if(ocr?.recipientMatched===false)return t("payment.recipientMismatch",{recipient}); return t("payment.recipientManual",{recipient}); }
function ocrMarkup(item={}) { const ocr=item.ocr&&typeof item.ocr==="object"?item.ocr:null; if(!ocr)return""; const message=ocrMessage(ocr); const recipient=recipientMessage(ocr); const status=["matched","mismatch","unreadable","manual_review"].includes(ocr.status)?ocr.status:"manual_review"; const icon=status==="matched"?"bi-check-circle":status==="mismatch"?"bi-exclamation-triangle":"bi-eye"; return `<div class="report-history-ocr ${status}"><i class="bi ${icon}" aria-hidden="true"></i><div class="report-history-ocr-copy"><span>${escapeHtml(message)}</span><small>${escapeHtml(recipient)}</small></div></div>`; }
function renderPaymentHistory(items=[]) { state.payments=items||[]; if(!els.paymentHistory)return; if(!state.payments.length){els.paymentHistory.innerHTML=`<div class="report-history-empty">${t("payment.empty")}</div>`;return;} els.paymentHistory.innerHTML=state.payments.map(item=>{ const date=item.submittedAt?new Date(item.submittedAt).toLocaleString(locale==="th"?"th-TH":"en-US",{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}):"-"; const status=["pending","approved","rejected"].includes(item.status)?item.status:"pending"; const deleteButton=status==="pending"?`<button class="btn btn-danger btn-sm" type="button" data-report-delete-slip="${escapeHtml(item.id)}"><i class="bi bi-trash"></i><span>${t("actions.deleteSlip")}</span></button>`:""; return `<article class="report-history-item"><div class="report-history-main"><strong>${escapeHtml(periodDateText(item.period?.label||"-"))}</strong><small>${escapeHtml(t("payment.submittedAt",{date}))}</small></div><div class="report-history-value"><span>${t("payment.amountDue")}</span><strong>${money(item.revenueShareAmount)} ${t("common.baht")}</strong></div><div class="report-history-value"><span>${escapeHtml(t("payment.rate",{rate:money(item.revenueShareRate)}))}</span><span class="report-status ${status}">${escapeHtml(statusLabel(status))}</span></div>${ocrMarkup(item)}${item.reviewNote?`<div class="report-history-note"><strong>${escapeHtml(t("payment.reviewNote"))}</strong><span>${escapeHtml(item.reviewNote)}</span></div>`:""}<div class="report-history-actions"><button class="btn btn-sm" type="button" data-report-view-slip="${escapeHtml(item.id)}"><i class="bi bi-receipt"></i><span>${t("actions.viewSlip")}</span></button>${deleteButton}</div></article>`; }).join(""); }
...
async function deletePendingSlip(item) {
  if (!item || item.status !== "pending") return;
  const period = periodDateText(item.period?.label || "-");
  if (!window.confirm(t("payment.deleteConfirm", { period }))) return;
  try {
    await callDeletePayment({ paymentId: item.id });
    closeSlip();
    setPaymentMessage(t("payment.deleteSuccess"));
    toast(t("payment.deleteSuccess"));
    await loadPaymentHistory();
  } catch (error) {
    console.error(error);
    toast(t("payment.deleteFailed"), "error");
    setPaymentMessage(t("payment.deleteFailed"), true);
  }
}
