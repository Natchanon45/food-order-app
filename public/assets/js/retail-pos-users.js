...
$("#permissionCheckboxes").addEventListener("click",event=>{const button=event.target.closest("[data-toggle-group]");if(!button)return;const card=button.closest(".permission-group-card"),inputs=[...card.querySelectorAll('input[type="checkbox"]')],next=!inputs.every(input=>input.checked);inputs.forEach(input=>input.checked=next);refreshPermissionSummary()});
$("#permissionCheckboxes").addEventListener("change",refreshPermissionSummary);
