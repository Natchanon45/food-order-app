...
export function padRunning(value, length = POS_RUNNING_PAD_LENGTH) { return String(Math.max(0, Number(value || 0))).padStart(length, '0'); }
...
export function monthKeyFrom(value = new Date()) { return dateKeyFrom(value).slice(0, 6); }
...
export function counterIdForRunningNumber({ type = 'SALE', periodKey = dateKeyFrom() } = {}) { return `${runningNumberConfig(type).type}_${periodKey}`; }
export function counterIdForDate(dateKey = dateKeyFrom()) { return counterIdForRunningNumber({ type: 'SALE', periodKey: dateKey }); }
export function buildRunningNumber({ prefix = POS_RUNNING_PREFIX, dateKey = dateKeyFrom(), running = 0, padLength = POS_RUNNING_PAD_LENGTH, separator = '-' } = {}) { return [prefix, dateKey, padRunning(running, padLength)].filter(Boolean).join(separator); }
...
export function pendingDocumentNumber({ type = 'SALE', value = new Date(), stableId = '' } = {}) {
  ...
  const clean = String(stableId || '').replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
  const suffix = clean.slice(-6) || Math.random().toString(16).slice(2, 8).toUpperCase();
  return [config.prefix, periodKey === 'all' ? '' : periodKey, 'PENDING', suffix].filter(Boolean).join('-');
}

export function buildCounterRow({ tenantId, type = 'SALE', dateKey = dateKeyFrom(), periodKey = '', running = 0, saleId = '', saleNumber = '', documentId = saleId, documentNumber = saleNumber, userId = '', now = Date.now() } = {}) {
  const config = runningNumberConfig(type);
  ...
  return { id, tenantId, shopId: tenantId, counterType: config.type === 'SALE' ? POS_COUNTER_TYPE : `${String(config.type).toLowerCase()}-number`, documentType: config.type, documentCollection: config.collection, channel: POS_CHANNEL, orderType: POS_ORDER_TYPE, prefix: config.prefix, reset: config.reset, dateKey, periodKey: resolvedPeriodKey, monthKey: String(dateKey).slice(0, 6), current: Number(running || 0), lastRunning: Number(running || 0), lastNumber: number, lastSaleId: saleId, lastDocumentId: documentId || saleId, lastDocumentNumber: number, schemaVersion: POS_FIRESTORE_VERSION, updatedBy: userId, updatedAt: now };
}
...
export function standardMeta({ tenantId, userId = '', now = Date.now(), deviceId = getDeviceId() } = {}) { return { tenantId, shopId: tenantId, deviceId, schemaVersion: POS_FIRESTORE_VERSION, deleted: false, createdAtMs: now, updatedAt: now, createdBy: userId, updatedBy: userId }; }
...
export function normalizeSaleForFirestore(sale = {}, { tenantId, userId = '', deviceId = getDeviceId(), now = Date.now() } = {}) {
  ...
  return { ...standardMeta({ tenantId, userId, now, deviceId }), ...sale, tenantId, shopId: tenantId, channel: sale.channel || POS_CHANNEL, orderType: sale.orderType || POS_ORDER_TYPE, dateKey, monthKey, paymentStatus: sale.paymentStatus || 'paid', status: sale.status || 'completed', syncStatus: sale.syncStatus || 'synced' };
}
...
export function buildSaleItemRows(sale = {}) {
  ...
  return (sale.items || []).map((item, index) => ({ id: `${saleId}_${item.productId || item.id || index}`.replace(/[^a-zA-Z0-9_-]/g, '_'), saleId, saleNumber, tenantId, shopId: tenantId, channel: POS_CHANNEL, orderType: POS_ORDER_TYPE, productId: item.productId || item.id || '', barcode: item.barcode || '', name: item.name || '', unit: item.unit || 'ชิ้น', qty: Number(item.qty || 0), price: Number(item.price || 0), cost: Number.isFinite(Number(item.cost)) ? Number(item.cost) : null, lineTotal: Number(item.lineTotal ?? Number(item.price || 0) * Number(item.qty || 0)), dateKey, createdAt }));
}
...
export function applySaleToDailySummary(summary = {}, sale = {}) {
  ...
  return { ...summary, id: sale.dateKey, tenantId: sale.tenantId, shopId: sale.tenantId, dateKey: sale.dateKey, monthKey: sale.monthKey, channel: POS_CHANNEL, totalSales: Number(summary.totalSales || 0) + total, totalAmount: Number(summary.totalAmount || 0) + total, totalDiscount: Number(summary.totalDiscount || 0) + discount, totalQty: Number(summary.totalQty || 0) + qty, billCount: Number(summary.billCount || 0) + 1, [`payment_${method}`]: Number(summary[`payment_${method}`] || 0) + total, updatedAt: Date.now() };
}

export function buildSyncQueueRow(sale = {}, { status = 'pending', error = '' } = {}) {
  return { id: sale.id, tenantId: sale.tenantId, shopId: sale.tenantId, saleId: sale.id, saleNumber: sale.saleNumber || '', channel: POS_CHANNEL, orderType: POS_ORDER_TYPE, syncStatus: status, retryCount: Number(sale.retryCount || 0), lastError: error, deviceId: sale.deviceId || getDeviceId(), updatedAt: Date.now() };
}
