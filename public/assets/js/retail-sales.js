...
import { sweetConfirm } from "./sweet-dialog.js?v=20260801-096";
...
function isVatSale(sale) {
  return (
    sale?.vatRegistered === true ||
    sale?.vatRegistered === "yes" ||
    Number(sale?.vatAmount || 0) > 0
  );
}
...
function filteredSales() {
  ...
  return sales.filter((sale) => {
    ...
    const items = (sale.items || [])
      .map(
        (item) => `${item.name || ""} ${item.id || ""} ${item.barcode || ""}`,
      )
      .join(" ")
      .toLowerCase();
    const matchesKeyword =
      !keyword ||
      saleDisplayNumber(sale).toLowerCase().includes(keyword) ||
      String(sale.id || "")
        .toLowerCase()
        .includes(keyword) ||
    ...
    return (
      ...
      (!from || date >= from) &&
      (!to || date <= to) &&
      (method === "all" || paymentMethod(sale) === method)
    );
  });
}
...
function buildProductRanking(rows) {
  ...
  rows.forEach((sale) =>
    (sale.items || []).forEach((item) => {
      const key = String(
        item.id || item.barcode || item.name || "ไม่ทราบสินค้า",
      );
...
function renderStats(rows) {
  ...
  const vatTotal = rows.reduce(
    (sum, sale) => sum + Number(sale.vatAmount || 0),
    0,
  );
  const vatBillCount = rows.filter(isVatSale).length;
  const cashTotal = rows
    .filter((sale) => paymentMethod(sale) === "cash")
    .reduce((sum, sale) => sum + saleTotalAmount(sale), 0);
  const transferTotal = rows
    .filter((sale) => paymentMethod(sale) !== "cash")
    .reduce((sum, sale) => sum + saleTotalAmount(sale), 0);
  const itemQtyTotal = rows.reduce(
    (sum, sale) =>
      sum +
      (sale.items || []).reduce(
  ...
  const highestSale = rows.reduce(
    (max, sale) => Math.max(max, saleTotalAmount(sale)),
    0,
  );
  ...
  if (els.beforeVatTotal)
    els.beforeVatTotal.textContent = money(beforeVatTotal);
  if (els.vatTotal) els.vatTotal.textContent = money(vatTotal);
  if (els.vatBillCount)
    els.vatBillCount.textContent = vatBillCount.toLocaleString("th-TH");
...
function renderBestSellers(rows) {
  ...
  els.bestSellerList.innerHTML = ranking
    .map(
      (item, index) =>
        `<article class="ranking-item"><div class="ranking-position">${index + 1}</div><div class="ranking-info"><strong>${escapeHtml(item.name)}</strong><span>${escapeHtml(item.id)}</span></div><div class="ranking-total"><strong>${item.qty.toLocaleString("th-TH")} ชิ้น</strong><span>${money(item.revenue)} บาท</span></div></article>`,
    )
    .join("");
}
...
function renderPeriodText() {
  ...
  if (from && to) {
    els.reportPeriodText.textContent =
      from === to ? `วันที่ ${from}` : `ตั้งแต่ ${from} ถึง ${to}`;
    return;
  }
  els.reportPeriodText.textContent = from
    ? `ตั้งแต่ ${from}`
    : `ถึงวันที่ ${to}`;
}
...
function renderSales() {
  ...
  els.salesTableBody.innerHTML = rows
    .map((sale) => {
...
function openSale(id) {
  ...
  els.receiptReceived.textContent = money(
    sale.payment?.received ?? sale.receivedAmount,
  );
  els.receiptChange.textContent = money(
    sale.payment?.change ?? sale.changeAmount,
  );
...
async function exportCsv() {
  const rows = filteredSales();
  if (!rows.length) {
    await sweetConfirm("ไม่มีข้อมูลสำหรับส่งออก", {
      title: "ส่งออก CSV",
      cancelText: "ยกเลิก",
      confirmText: "ตกลง",
      cancelIcon: "x-circle",
  ...
  rows.forEach((sale) => {
    ...
    lines.push([
      ...
      new Date(sale.createdAt || Date.now()).toLocaleString("th-TH"),
...
[els.saleSearch, els.dateFrom, els.dateTo, els.paymentFilter].forEach(
  (element) =>
...
els.printReceiptBtn.addEventListener("click", () => {
  ...
  window.open(
    receiptPrintUrl(sale),
    `pos_receipt_${String(sale.id || saleDisplayNumber(sale)).replace(/[^a-zA-Z0-9]/g, "_")}`,
...
renderSales();
const stopSalesWatch = watchRecords(
  RetailCollections.sales,
  (rows) => {
    sales = rows;
