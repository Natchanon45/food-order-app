...
function nextActions(status, orderType) {
  ...
  if (status === "cooking") return [["ready", orderType === "delivery" ? "พร้อมจัดส่ง" : orderType === "takeaway" ? "พร้อมเสิร์ฟ" : "พร้อมเสิร์ฟ", "btn-primary"]];
...
function lockedItemLabel(order) { if (isDelivery(order) && order?.status === "served") return "ส่งให้ไรเดอร์แล้ว"; if (order?.status === "served") return "เสิร์ฟแล้ว"; if (order?.status === "paid") return "ชำระเงินแล้ว"; return "ล็อกแล้ว"; }
...
function openEditor(order, itemIndex) {
  const item = order.items?.[itemIndex]; if (!item || item.cancelled || isKitchenLocked(order)) return;
  const maxQty = Math.max(1, Number(item.originalQty || item.qty || 1)); const activeMenus = availableMenus.filter(menu => menu.active !== false); const options = activeMenus.map(menu => `<option value="${menu.id}" ${menu.id === item.menuId ? "selected" : ""}>${menu.name} — ${money(menu.price)} บาท</option>`).join("");
  const backdrop = document.createElement("div"); backdrop.className = "kitchen-item-editor-backdrop"; backdrop.innerHTML = `<section class="kitchen-item-editor"><h2>แก้ไขรายการ</h2><div class="editor-grid"><div class="field"><label>เมนู</label><select class="input" id="kitchenReplacementMenu">${options}</select></div><div class="field"><label>จำนวนที่ทำได้</label><input class="input" id="kitchenReplacementQty" type="number" min="1" max="${maxQty}" step="1" value="${Math.min(Number(item.qty || 1), maxQty)}"></div><div class="field"><label>หมายเหตุการแก้ไข *</label><input class="input" id="kitchenReplacementNote" value="${item.note || ""}" maxlength="200"></div></div><div class="editor-actions"><button type="button" class="btn" data-close-editor>ยกเลิก</button><button type="button" class="btn btn-primary" data-save-editor>บันทึกการแก้ไข</button></div></section>`;
  backdrop.addEventListener("click", async event => { if (event.target === backdrop || event.target.closest("[data-close-editor]")) { closeEditor(); return; } const saveButton = event.target.closest("[data-save-editor]"); if (!saveButton || isKitchenLocked(order)) return; const selectedMenu = activeMenus.find(menu => menu.id === backdrop.querySelector("#kitchenReplacementMenu").value); const nextQty = Number(backdrop.querySelector("#kitchenReplacementQty").value); const nextNote = backdrop.querySelector("#kitchenReplacementNote").value.trim(); if (!selectedMenu || !Number.isInteger(nextQty) || nextQty < 1 || nextQty > maxQty) { toast("กรุณาตรวจสอบเมนูและจำนวน", "error"); return; } if ((nextQty !== Number(item.qty) || selectedMenu.id !== item.menuId) && !nextNote) { toast("กรุณาระบุหมายเหตุก่อนแก้ไข", "error"); return; } const items = order.items.map((row, index) => index !== itemIndex ? row : { ...row, originalQty: row.originalQty || row.qty, originalName: row.originalName || row.name, menuId: selectedMenu.id, name: selectedMenu.name, price: Number(selectedMenu.price), qty: nextQty, note: nextNote, replacedFromName: selectedMenu.id !== row.menuId ? (row.replacedFromName || row.name) : row.replacedFromName || "", updatedByKitchenAt: new Date().toISOString() }); const totals = recalculateOrder(order, items); saveButton.disabled = true; try { await dataService.updateOrder(order.id, { items, ...totals, kitchenAdjustedAt: new Date().toISOString() }); closeEditor(); toast("อัปเดตรายการและคำนวณยอดใหม่แล้ว"); } catch (error) { console.error(error); toast("แก้ไขรายการไม่สำเร็จ", "error"); saveButton.disabled = false; } });
