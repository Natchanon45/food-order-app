...
function renderCart() {
  ...
  els.cartList.innerHTML = cart.map(item => `<div class="cart-row"><div><div class="cart-name">${escapeHtml(item.name)}</div><div class="cart-meta">${money(item.price)} บาท / ${escapeHtml(item.unit)}</div><div class="qty-tools"><button type="button" data-action="decrease" data-id="${escapeHtml(item.id)}" aria-label="ลดจำนวน ${escapeHtml(item.name)}" ${savingSale ? "disabled" : ""}>−</button><strong>${item.qty}</strong><button type="button" data-action="increase" data-id="${escapeHtml(item.id)}" aria-label="เพิ่มจำนวน ${escapeHtml(item.name)}" ${savingSale ? "disabled" : ""}>+</button><button type="button" class="remove qty-remove" data-action="remove" data-id="${escapeHtml(item.id)}" aria-label="ลบ ${escapeHtml(item.name)}" title="ลบสินค้า" ${savingSale ? "disabled" : ""}><i class="bi bi-trash3" aria-hidden="true"></i></button></div></div><div class="line-total">${money(item.price * item.qty)}</div></div>`).join("");
