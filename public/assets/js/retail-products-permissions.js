import {hasPermission} from "./retail-pos-navigation.js?v=20260713-003";

const ACTIONS={
  create:"pos.products.create",
  edit:"pos.products.edit",
  remove:"pos.products.delete",
...
function applyVisibility(){
  ...
  document.querySelectorAll('[data-action="delete"]').forEach(button=>setHidden(button,!hasPermission(ACTIONS.remove)));
  ...
  setHidden(document.querySelector("#productCost")?.closest("label"),!canViewCost);
  if(!canViewCost){
...
function requiredPermission(target){
  ...
  if(actionButton.dataset.action==="delete")return ACTIONS.remove;
  return ACTIONS[actionButton.dataset.action]||null;
}

document.addEventListener("click",event=>{const permission=requiredPermission(event.target);if(permission&&!hasPermission(permission)){event.preventDefault();event.stopImmediatePropagation();deny()}},true);
document.addEventListener("submit",event=>{if(event.target.matches("#productForm")){const editing=Boolean(document.querySelector("#editingProductId")?.value);const permission=editing?ACTIONS.edit:ACTIONS.create;if(!hasPermission(permission)){event.preventDefault();event.stopImmediatePropagation();deny()}}if(event.target.matches("#stockForm")&&!hasPermission(ACTIONS.stock)){event.preventDefault();event.stopImmediatePropagation();deny()}},true);

const tableBody=document.querySelector("#productTableBody");
if(tableBody){let scheduled=false;new MutationObserver(()=>{if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;applyVisibility()})}).observe(tableBody,{childList:true})}
applyVisibility();
