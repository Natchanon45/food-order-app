...
if (usingDemoMode) {
  document.querySelector("#demoBanner").innerHTML = '<div class="demo-banner">โหมดตัวอย่าง: การขายจะถูกบันทึกในเบราว์เซอร์นี้</div>';
}
...
function productStock(product) {
  return Math.max(0, Number(product.stock ?? product.qty ?? 0));
}
...
function renderProducts() {
  ...
  productGrid.innerHTML = filtered.length ? filtered.map(product => {
    ...
    return `<article class="card pos-product-card" aria-disabled="${stock <= 0}">
      <div class="pos-product-meta"><span>${product.category || "ทั่วไป"}</span><span>${product.sku || product.code || ""}</span></div>
      <h3>${product.name || "ไม่ระบุชื่อ"}</h3>
      <div class="pos-product-price">${money(product.price || 0)} บาท</div>
      <div class="pos-product-meta"><span class="${stockClass}">คงเหลือ ${stock}</span><button class="btn btn-primary btn-sm" data-add-product="${product.id}" ${stock <= 0 ? "disabled" : ""}>เพิ่ม</button></div>
    </article>`;
...
function renderCart() {
  const rows = [...cart.values()];
  cartRows.innerHTML = rows.length ? rows.map(row => `
    <div class="pos-cart-row">
      <div class="pos-cart-head"><strong>${row.product.name}</strong><button class="btn btn-danger btn-sm" data-remove-product="${row.product.id}">ลบ</button></div>
      <div class="pos-cart-controls">
        <div class="pos-qty"><button class="btn btn-sm" data-decrease-product="${row.product.id}">−</button><strong>${row.qty}</strong><button class="btn btn-sm" data-increase-product="${row.product.id}">+</button></div>
        <strong>${money(row.qty * Number(row.product.price || 0))} บาท</strong>
      </div>
    </div>`).join("") : '<div class="empty">ยังไม่มีสินค้าในรายการขาย</div>';
...
function addProduct(id) {
  ...
  if (current >= productStock(product)) {
    toast("จำนวนสินค้าเกินสต็อกคงเหลือ", "error");
    return;
  }
...
checkoutButton.addEventListener("click", async () => {
  ...
  try {
    const result = await dataService.completeSale({
    ...
    console.error(error);
    const message = error.message === "INSUFFICIENT_STOCK" ? `สต็อก ${error.productName || "สินค้า"} ไม่เพียงพอ` : "บันทึกการขายไม่สำเร็จ";
    toast(message, "error");
  } finally {
