...
export const MENU_GROUPS=[
  ...
  {id:"stock",label:"สินค้าและสต็อก",items:[{key:"pos.products",label:"สินค้าและสต็อก",href:"/pos/products/"},{key:"pos.stock_movements",label:"เคลื่อนไหวสต็อก",href:"/pos/stock-movements/"},{key:"pos.stock_counts",label:"ตรวจนับสต็อก",href:"/pos/stock-counts/"}]},
  {id:"purchase",label:"จัดซื้อ",items:[{key:"pos.purchases",label:"รับสินค้าเข้า",href:"/pos/purchases/"},{key:"pos.payables",label:"เจ้าหนี้",href:"/pos/payables/"},{key:"pos.suppliers",label:"ผู้จำหน่าย",href:"/pos/suppliers/"}]},
  {id:"customer",label:"ลูกค้าและสมาชิก",items:[{key:"pos.customers",label:"ทะเบียนลูกค้า",href:"/pos/customers/"}]},
  {id:"system",label:"ระบบ",items:[{key:"pos.settings",label:"ตั้งค่าร้าน",href:"/pos/settings/"},{key:"pos.backup",label:"สำรองและกู้คืน",href:"/pos/backup/"},{key:"pos.users",label:"ผู้ใช้และสิทธิ์",href:"/pos/users/"}]}
];
...
export function ensureAccessData(){const roles=read(ROLE_KEY,[]);if(!roles.length){write(ROLE_KEY,DEFAULT_ROLES);localStorage.setItem(MIGRATION_KEY,"1")}else migrateRolesOnce(roles);ensureAuthUsers()}
