...
function isTableOrder(order) { return order?.orderType !== "delivery" && order?.orderType !== "takeaway"; }
...
function renderTableBill(group) {
  ...
  const unpaidPaymentRounds = sorted.filter(order => order.paymentStatus !== "paid");
  const servedCount = sorted.filter(isServed).length;
  const paymentBadge = unpaidPaymentRounds.length ? `<span class="badge warning">รอชำระเงิน</span>` : `<span class="badge">ชำระแล้ว รอเสิร์ฟครบ</span>`;
  ...
  const paymentAction = unpaidPaymentRounds.length ? `<button class="btn btn-primary" data-table-payment="${key}">${icon("easel2")}<span>ตรวจแล้ว/ชำระแล้ว</span></button>` : `<button class="btn btn-primary" disabled>${icon("check-circle")}<span>ชำระแล้ว</span></button>`;
...
grid.addEventListener("click", async event => {
  ...
  if (tablePaymentButton) { const key = tablePaymentButton.dataset.tablePayment; const rounds = activeOrders(currentOrders).filter(order => isTableOrder(order) && tableGroupKey(order) === key); const payableRounds = rounds.filter(order => order.paymentStatus !== "paid"); if (!rounds.length || !payableRounds.length) return; const total = payableRounds.reduce((sum, order) => sum + Number(order.totalAmount || 0), 0); const ok = await askConfirm(`รับชำระเงินโต๊ะ ${rounds[0].tableCode} จำนวน ${money(total)} บาท ใช่หรือไม่?

ถ้ายังเสิร์ฟไม่ครบ รายการจะยังอยู่ที่ครัวและแคชเชียร์จนกว่าจะเสิร์ฟครบ`, { title: "รับชำระเงิน", confirmText: "ตกลง", cancelText: "ยกเลิก", type: "warning" }); if (!ok) return; tablePaymentButton.disabled = true; try { const now = new Date().toISOString(); await Promise.all(payableRounds.map(order => dataService.updateOrder(order.id, tablePaymentPatch(order, now)))); const allWillBeClosed = rounds.every(order => order.status === "paid" || isServed(order)); if (allWillBeClosed) await closeTableAfterPayment(rounds); toast(allWillBeClosed ? `รับชำระและปิดบิลโต๊ะ ${rounds[0].tableCode} เรียบร้อย` : `รับชำระโต๊ะ ${rounds[0].tableCode} แล้ว รอครัวเสิร์ฟให้ครบก่อนปิดบิล`); } catch (error) { console.error(error); toast("รับชำระรวมบิลไม่สำเร็จ", "error"); tablePaymentButton.disabled = false; } return; }
