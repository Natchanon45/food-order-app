...
Retail POS navigation rule: submenu expand/collapse controls must render Bootstrap Icons chevrons, not text carets such as `^`, `v`, `⌃`, or `⌄`. The drawer title `เมนู POS` must not receive an injected context icon.
...
Completed in this build: Retail POS drawer title icon cleanup with navigation/icon cache bump and browser verification.
