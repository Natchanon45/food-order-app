...
Compact QR Pairing workflow: the PC opens `/pos/customer-display?displayId=display-pc-01`; the header shows a small Bootstrap `bi-qr-code` icon with `เชื่อมอุปกรณ์`; hovering or focusing the control reveals the full QR panel for `/pos?registerId=iphone-01&displayId=display-pc-01`; the selling device scans that QR, opens POS, stores the display config through the existing Multi Register flow, and publishes sales back to `customerDisplays/display-pc-01`.

Completed in this build: hover-only Customer Display QR Pairing refinement.
