...
Full Tax Invoice phase 1 rule: full tax invoices are stored separately from sales in `taxInvoices/{taxInvoiceId}` and include `tenantId`, source sale reference, seller tax profile, buyer tax profile, line items, VAT summary, total, status, and issued timestamp. The receipt popup can create/reuse one full tax invoice per sale and opens `/pos/tax-invoice/?invoiceId=...` for A4 printing. This phase does not change POS sale totals, VAT calculation, stock deduction, offline sale sync, or existing short tax invoice receipt behavior.

Completed in this build: P9-B006 Full Tax Invoice phase 1 service, record creation, receipt action, and print page.

Next task: improve P9-B006 with a proper buyer tax information modal, customer tax profile reuse, tax invoice history/reprint UI, and optional tax invoice running number counter.
